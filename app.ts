...
let CurrentClientWorkouts: Array<Workout> = new Array<Workout>
let Clients: Array<Client> = new Array<Client>
...
function RestoreWorkoutData(fClientId: number) {
    ...
    for (var i = 0; i < CurrentClientWorkouts.length; i++) {
        console.log(`ClientID ${CurrentClientWorkouts[i].clientId}. WorkoutType ${CurrentClientWorkouts[i].Exercise}`)
    }
}
